...
- Brand assets (favicon, monogram, any mark derived from `src/assets/logo*`) must faithfully reproduce the source image — no hand-approximated curves. If no vector source exists, trace the raster (e.g. the `potrace` npm package) rather than eyeballing it. Never write task language that waives this ("não precisa ser pixel-perfect" or similar) — ask the user first if a task file already contains that kind of allowance.
